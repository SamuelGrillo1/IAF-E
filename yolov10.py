...
def update_objects(objects, detected_boxes, debounce_frames=30):
    current_time = time.time()
    new_objects = {}
    global total_counter

    # Update positions or create new for current detections
    ...
    for obj_id, obj in objects.items():
        if obj_id in new_objects:
            objects[obj_id] = new_objects[obj_id]
        else:
            # If not updated, check if it should disappear
            if current_time - obj.last_detected_time > debounce_frames:
                ...
                if not obj.disappeared_once:
                    ...
                    total_counter += 1
                    print(f"Object {obj_id} has disappeared.")
                    
    return objects

def draw_detections(frame, detected_boxes, objects):
    total_counter = len([obj for obj in objects.values() if obj.disappeared_once])
    for obj_id, obj in objects.items():
    ...
    # Display total count of disappeared objects
    total_label = f"Total de itens detectados: {total_counter}"
    label_size, _ = cv2.getTextSize(total_label, cv2.FONT_HERSHEY_SIMPLEX, 1.0, 2)
    ...
    cv2.putText(frame, total_label, (label_x, label_y + label_size[1]), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 255), 2)

    return frame

torch.set_default_dtype(torch.float32)
model = YOLO(r'D:\Mills\ProjectMills\IAF&E\IA-Esteira\modelo\detect4\content\runs\detect\train2\weights\best.pt')
...
while True:
    ...
    objects = update_objects(objects, detected_boxes)
