...
def update_total_label(frame, total_count):
    total_label = f"Total de itens detectados até agora: {total_count}"
...
for filename in os.listdir(folder_path):
    ...
    if filename.endswith('.MOV') or filename.endswith('.mp4'):
        ...
        out = cv2.VideoWriter(save_path, fourcc, fps, (int(cap.get(3)), int(cap.get(4))))
        ...
        while(cap.isOpened()):
            ...
            if ret:
                ...
                processed_frame = update_total_label(processed_frame, total_count)
